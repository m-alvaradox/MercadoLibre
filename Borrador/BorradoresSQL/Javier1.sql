...
BEGIN
	...
	CASE DATO_ACTUALIZAR
		WHEN 'USERID' THEN
            ...
            SELECT USERID FROM USUARIO WHERE USERID = IDUSUARIO INTO valor_nuevo;
		WHEN 'EMAIL' THEN
...
BEGIN
	...
	CASE CAMPO
		WHEN 'nombrepublicacion' THEN
			UPDATE PUBLICACION SET NOPUBLICACION =  NUEVO_VALOR WHERE NOPUBLICACION = IDPUBLICACION;
			SELECT NOPUBLICACION FROM PUBLICACION WHERE NOPUBLICACION = IDPUBLICACION INTO valor_nuevo;
...
-- En una aplicacion como mercado libre es muy importante el país, por lo que cada busqueda o uso en el sistema será optimizado al crear un indice de la tabla país
